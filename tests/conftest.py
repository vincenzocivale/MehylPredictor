...
@pytest.fixture()
def synthetic_pretrained_project(tmp_path: Path) -> tuple[Path, Path]:
    """An F2 config (encoder=linear/interaction=concat) and a matching nested
    pretrained-embedding config (encoder=pretrained_embedding/interaction=cpg_pretrained_f2,
    pretrained_mode=concat) sharing the same RNA/beta/locus/metadata fixtures, for testing
    real warm-start + freeze + epoch-0-checkpoint behavior through ExperimentRunner.train().
    Returns (f2_config_path, nested_config_path).
    """
    rng = np.random.default_rng(11)
    n_samples, n_genes, n_cpgs, locus_dim, pretrained_dim = 24, 16, 12, 8, 5
    sample_ids = np.asarray([f"S{i:03d}" for i in range(n_samples)], dtype=object)
    cpg_ids = np.asarray([f"cg{i:05d}" for i in range(n_cpgs)], dtype=object)
    gene_ids = np.asarray([f"G{i:04d}" for i in range(n_genes)], dtype=object)
    cancer_types = np.asarray(["A", "B", "C"] * 8, dtype=object)
    ...
    rna = rng.normal(size=(n_samples, n_genes)).astype(np.float32)
    pretrained = rng.normal(size=(n_samples, pretrained_dim)).astype(np.float32)
    embeddings = rng.normal(size=(n_cpgs, locus_dim)).astype(np.float32)
    prior_logit = rng.normal(scale=0.8, size=n_cpgs)
    prior = (1 / (1 + np.exp(-prior_logit))).astype(np.float32)
    sample_factor = rna[:, :4] @ rng.normal(size=(4, 3))
    ...
    with h5py.File(tmp_path / "rna.h5", "w") as handle:
        ...
        handle.create_dataset("sample_idx", data=sample_ids, dtype=string_dtype)
        handle.create_dataset("gene_ids", data=gene_ids, dtype=string_dtype)
    with h5py.File(tmp_path / "pretrained.h5", "w") as handle:
        handle.create_dataset("embeddings", data=pretrained)
        handle.create_dataset("sample_idx", data=sample_ids, dtype=string_dtype)
        handle.create_dataset("feature_ids", data=np.asarray([f"p{i}" for i in range(pretrained_dim)], dtype=object), dtype=string_dtype)
    with h5py.File(tmp_path / "beta.h5", "w") as handle:
        handle.create_dataset("beta", data=beta)
        handle.create_dataset("sample_idx", data=sample_ids, dtype=string_dtype)
        handle.create_dataset("cpg_idx", data=cpg_ids, dtype=string_dtype)
    ...
    f2_config = {
        "run_name": "synthetic_f2",
        "output_dir": str(tmp_path / "run_f2"),
        "data": dict(shared_data),
        "model": {
            "encoder": {"kind": "linear", "latent_dim": 8, "dropout": 0.0},
            "interaction": {"kind": "concat", "hidden_dim": 8, "dropout": 0.0},
            "gate": {"kind": "global", "hidden_dim": 8},
            "anchor_to_mean_rna": True,
            "zero_init_residual": True,
        },
        "loss": {"residual_huber_weight": 0.05, "shrinkage_weight": 1e-5},
        "training": {
            "seed": 13, "device": "cpu", "epochs": 3, "steps_per_epoch": 8,
            "sample_batch_size": 8, "cpg_batch_size": 6, "learning_rate": 0.001,
            "weight_decay": 0.0, "amp": False, "patience": 3, "validation_max_cpgs": None,
        },
        "evaluation": shared_evaluation,
    }
    f2_path = tmp_path / "config_f2.yaml"
    f2_path.write_text(yaml.safe_dump(f2_config, sort_keys=False))

    nested_data = dict(shared_data)
    nested_data["pretrained_rna"] = {
        "path": str(tmp_path / "pretrained.h5"), "values_key": "embeddings",
        "row_ids_key": "sample_idx", "col_ids_key": "feature_ids",
    }
    nested_data["pretrained_mode"] = "concat"
    nested_config = {
        "run_name": "synthetic_nested",
        "output_dir": str(tmp_path / "run_nested"),
        "data": nested_data,
        "model": {
            "encoder": {"kind": "pretrained_embedding", "latent_dim": 8, "dropout": 0.0},
            "interaction": {"kind": "cpg_pretrained_f2", "hidden_dim": 8, "dropout": 0.0},
            "gate": {"kind": "global", "hidden_dim": 8},
            "anchor_to_mean_rna": True,
            "zero_init_residual": True,
        },
        ...
        "training": {
            ...
            "sample_batch_size": 8, "cpg_batch_size": 6, "learning_rate": 0.001,
            "weight_decay": 0.0, "amp": False, "patience": 3, "validation_max_cpgs": None,
            "warm_start_checkpoint": str(tmp_path / "run_f2" / "best.pt"),
            "freeze_warm_start_params": True, "seed_initial_checkpoint": True,
        },
        "evaluation": shared_evaluation,
    }
    nested_path = tmp_path / "config_nested.yaml"
    nested_path.write_text(yaml.safe_dump(nested_config, sort_keys=False))
    return f2_path, nested_path
