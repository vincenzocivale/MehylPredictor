...
def test_film_locus_interaction_shapes():
    config = ModelConfig(
        encoder=EncoderConfig(kind="linear", latent_dim=8, dropout=0.0),
        interaction=InteractionConfig(kind="film_locus", hidden_dim=12, dropout=0.0),
        gate=GateConfig(kind="global", initial_global_scale=1.0),
        anchor_to_mean_rna=True,
        zero_init_residual=False,
    )
    model = ResidualMethylationModel(10, 6, config)
    rna = torch.randn(3, 10)
    reference = torch.zeros(1, 10)
    loci = torch.randn(5, 6)
    prior = torch.linspace(0.1, 0.9, 5)
    variability = torch.randn(5, 2)
...
def test_linear_tokens_cross_attention_requires_matching_encoder():
    config = ModelConfig(
        encoder=EncoderConfig(kind="linear", latent_dim=8, dropout=0.0),
        interaction=InteractionConfig(kind="linear_token_cross_attention", hidden_dim=12, dropout=0.0),
        gate=GateConfig(kind="global", initial_global_scale=1.0),
    )
    try:
        ResidualMethylationModel(10, 6, config)
        assert False, "expected ValueError for mismatched encoder/interaction pairing"
    except ValueError:
        pass


@pytest.mark.parametrize("kind", ["concat_only", "product_only", "concat_product_linear", "interaction_mlp", "raw_concat"])
def test_stage_g_ablation_interactions_shapes(kind: str):
    interaction_kwargs = {"hidden_dim": 12, "dropout": 0.0} if kind != "concat_product_linear" else {"dropout": 0.0}
    config = ModelConfig(
        encoder=EncoderConfig(kind="linear", latent_dim=8, dropout=0.0),
        interaction=InteractionConfig(kind=kind, mlp_hidden_dims=[16], **interaction_kwargs),
        gate=GateConfig(kind="global", initial_global_scale=1.0),
        anchor_to_mean_rna=True,
        zero_init_residual=False,
    )
...
def test_linear_tokens_cross_attention_shapes():
    config = ModelConfig(
        encoder=EncoderConfig(kind="linear_tokens", token_dim=4, num_latents=3, dropout=0.0),
        interaction=InteractionConfig(kind="linear_token_cross_attention", hidden_dim=12, dropout=0.0),
        gate=GateConfig(kind="global", initial_global_scale=1.0),
        anchor_to_mean_rna=True,
        zero_init_residual=False,
