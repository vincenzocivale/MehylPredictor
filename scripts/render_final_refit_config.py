#!/usr/bin/env python3
"""Render the final-refit config from the development config plus its
measured `best_epoch`, via programmatic YAML edit (not sed) to avoid
fragile text surgery.

Per the 2-stage protocol: same architecture, hyperparameters, preprocessing,
optimizer, schedule, seed, and epoch definition as the development stage --
the ONLY changes are (1) data.sample_metadata/cpg_splits point back at the
ORIGINAL, unmodified manifests (100% of official train_cpg/train_sample, not
the 90% dev_fit pool), (2) epochs/min_epochs are fixed to best_epoch and
...
def render(dev_config_path: Path, best_epoch: int, output_path: Path) -> Path:
    ...
    raw["run_name"] = "train-final-seed17"
    raw["output_dir"] = "artifacts/train/seed17/final_refit"
    ...
    raw["tracking"]["name"] = "train-final-seed17"
    tags = raw["tracking"].get("tags", [])
    raw["tracking"]["tags"] = [t for t in tags if t != "development"] + ["final-refit"]
