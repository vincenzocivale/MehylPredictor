...
def main():
    p=argparse.ArgumentParser(description=__doc__); p.add_argument("--model",choices=["rna_methylation","cpg_statistics","cpg_prior"],required=True); p.add_argument("--checkpoint"); p.add_argument("--eval-scope",choices=["chr1","chr123","genomewide"],required=True); p.add_argument("--output",required=True); p.add_argument("--registry",required=True); p.add_argument("--canonical-root"); p.add_argument("--feature-cache",help="legacy CpG-prior baseline cache"); p.add_argument("--prior-cache",help="RNA metric-only prior cache (cpg_idx.npy + prior.npy)"); p.add_argument("--rna-cache"); p.add_argument("--locus-store"); p.add_argument("--functional-atlas"); p.add_argument("--annotation-cache"); p.add_argument("--genomic-fm-cache",help="E04 comparator: chr1-only genomic-FM embedding cache, mutually exclusive with --locus-store/--functional-atlas/--annotation-cache"); p.add_argument("--targets"); p.add_argument("--embeddings"); p.add_argument("--sample-chunk",type=int,default=128); p.add_argument("--cpg-chunk",type=int,default=2048); p.add_argument("--prepared-root",help="matched chr1 data root; omit for canonical chr123"); p.add_argument("--cpg-targets-dir",help="training-locus mean-proxy targets dir"); p.add_argument("--recipe",default="configs/models/main.yaml",help="RNA model recipe; defaults to the paper-facing reference configuration"); args=p.parse_args()
    if args.model=="rna_methylation":
        from methylation_predictor.rna_training import evaluate_rna_checkpoint
        if args.checkpoint is None: p.error("--checkpoint is required for RNA evaluation")
        for name in ("canonical_root","prior_cache","rna_cache"):
            if getattr(args,name) is None: p.error(f"--{name.replace('_','-')} is required for RNA evaluation")
        if not args.genomic_fm_cache and not args.locus_store and not (args.functional_atlas and args.annotation_cache): p.error("--locus-store, --genomic-fm-cache, or both legacy cache paths are required")
        if args.eval_scope == "chr1" and args.prepared_root is None: p.error("--prepared-root is required for matched chr1 evaluation")
        result=evaluate_rna_checkpoint(checkpoint=args.checkpoint,canonical_root=args.canonical_root,recipe_path=args.recipe,rna_cache=args.rna_cache,prior_cache=args.prior_cache,registry=args.registry,matched_chr1_root=args.prepared_root,cpg_targets_dir=args.cpg_targets_dir,output=args.output,scope=args.eval_scope,sample_chunk=args.sample_chunk,cpg_chunk=args.cpg_chunk,functional_atlas=args.functional_atlas,annotation_cache=args.annotation_cache,locus_store=args.locus_store,genomic_fm_cache=args.genomic_fm_cache)
