#!/usr/bin/env python3
"""Fase 1 preflight for the genome-wide concat full-coverage run: re-verifies
official-split integrity/overlap, data completeness (embeddings/prior/
variability/beta/RNA), disk/RAM/GPU headroom, runs the existing test suite,
and does a brief smoke run of the real full-coverage training path (uniform
sampling, 1 epoch, 3 steps -- fast, not the full deterministic schedule) on
the real (dev-split) data paths. Writes a manifest with sha256 of every input
checked. Exits non-zero on any hard failure; a soft-failure test (the
pre-existing, unrelated rna_encoder_quality failure) is recorded but does not
block the run.
"""
...
RNA_BRANCH_INPUTS = SCRATCH / "rna_branch_inputs"
RNA_BRANCH_INPUTS_DEV = SCRATCH / "rna_branch_inputs_dev_seed17"
...
def check_tests() -> dict:
    ...
    passed = "failed" not in tail or "1 failed" in tail  # the one known pre-existing failure is tolerated
    return {
        "returncode": result.returncode,
        "tail": tail,
        "known_pre_existing_failure_only": "1 failed" in tail and "rna_encoder_quality" in result.stdout,
    }
...
def check_official_splits() -> dict:
    ...
    cpg_split_manifest = pd.read_parquet(RNA_BRANCH_INPUTS / "cpg_split_manifest.parquet")
    sample_metadata = pd.read_parquet(RNA_BRANCH_INPUTS / "sample_metadata.parquet")

    from methylation_predictor.common.splits import assert_disjoint
    assert_disjoint(cpg_split_manifest, "cpg_idx", "split")
    assert_disjoint(sample_metadata, "sample_idx", "split")
...
def check_data_completeness() -> dict:
    with h5py.File(RNA_BRANCH_INPUTS / "locus_embeddings.h5", "r") as f:
        embeddings_shape = f["embeddings"].shape
    locus_features = pd.read_parquet(RNA_BRANCH_INPUTS / "locus_features.parquet")
    with h5py.File(RNA_BRANCH_INPUTS / "tcga_genome_wide_beta.h5", "r") as f:
        beta_shape = f["beta"].shape
        sample_chunk = f["beta"][:200, :5000]
    with h5py.File(RNA_BRANCH_INPUTS / "tcga_rna.h5", "r") as f:
        rna_shape = f["X"].shape
...
def check_dev_split() -> dict:
    manifest_path = RNA_BRANCH_INPUTS_DEV / "manifest.json"
...
def smoke_run(output_dir: Path) -> dict:
    ...
    dev_config_path = REPO_ROOT / "configs/rna_branch/tcga_genome_wide_concat_dev_seed17.yaml"
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=REPO_ROOT / "artifacts/rna_branch/concat_genomewide_fullcoverage_seed17_v1/manifests/preflight.json",
    )
    ...
    if not args.skip_tests:
        ...
        if report["tests"]["returncode"] != 0 and not report["tests"]["known_pre_existing_failure_only"]:
            ok = False
