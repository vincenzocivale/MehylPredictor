...
`train` label of the already-prepared genome-wide rna_branch inputs, for the
2-stage protocol's Stage 3A (development training). Never touches the
...
"train")`) -- pointing a 3A config at these new files means the standardizer,
reference RNA vector, and variability-tertile thresholds are automatically
and correctly fit on `dev_fit` (=90% train) only, with zero changes needed
to trainer.py/data.py/config.py beyond what full-coverage training already
required.
...
of this CpG's held-out status in Stage 3A." Re-fitting the OOF ensemble for
every candidate nested split would be substantial extra engineering for a
second-order effect; this script does not attempt it.
"""
...
def main() -> None:
    ...
    assert_disjoint(cpg_dev.rename(columns={"cpg_idx": "key"}), "key", "split")
    assert_disjoint(sample_dev.rename(columns={"sample_idx": "key"}), "key", "split")
    ...
    manifest = {
        ...
        "downstream_usage": (
            "Stage 3A (development training) config points data.cpg_splits/data.sample_metadata "
            "at these _dev files, with training.train_{cpg,sample}_split=train (=dev_fit, 90%) "
            "and training.validation_{cpg,sample}_split=dev_heldout (10%). Stage 3B (final refit) "
