...
ORCH_LOG="$ROOT/logs/functional_concat_final/efficient_8ffn_orchestrator.log"
LOGDIR="$ROOT/logs/functional_concat_final"
mkdir -p "$LOGDIR"

echo "[eval] waiting for training to finish (watching $ORCH_LOG)..."
while ! grep -q "efficient_single_attn_8ffn_residual finished successfully" "$ORCH_LOG" 2>/dev/null; do
  if grep -q "FAILED" "$ORCH_LOG" 2>/dev/null; then
    echo "[eval] training FAILED per orchestrator log; not evaluating." | tee -a "$LOGDIR/eval_j7_orchestrator.log"
    exit 1
  fi
  sleep 30
done
...
echo "[eval] evaluation finished, see $RUN_DIR/evaluation/chr1/official_split.json" | tee -a "$LOGDIR/eval_j7_orchestrator.log"
