...
RESULTS = REPO_ROOT / "results/reference/appendix/ablations/rna_encoder_comparison_2026_09"
ARMS = ("gene_pathway", "bulkformer_147m", "enc_bottleneck_mlp")
...
def main():
    ap = argparse.ArgumentParser(description=__doc__); ap.add_argument("--data-root"); ap.add_argument("--scope", choices=["chr1", "chr123"], default="chr1"); ap.add_argument("--mode", choices=["development", "final"], default="development"); ap.add_argument("--seed", type=int, default=17); args = ap.parse_args()
    base = root(args.data_root) / "experiments/runs/locus_cls_joint" / args.scope; rows = []
    for arm in ARMS:
        ...
        if args.mode == "development":
            ...
            ev = load(rd / "evaluation" / args.scope / "metrics.json")
            if not ev: continue
    ...
    scope_suffix = "" if args.scope == "chr1" else f"_{args.scope}"
    payload = {"study": "rna_encoder_comparison_2026_09", "protocol": {"scope": f"TCGA {args.scope}", "mode": args.mode, "seed": args.seed, "fixed_harness": "mean branch ON; raw product OFF; same loss/training budget"}, "runs": rows, "reported_context": {"methylprophet_gene_pathway_table9": {"train_cpg_x_val_sample_mas_pcc": 0.5371, "val_cpg_x_train_sample_mas_pcc": 0.4194, "val_cpg_x_val_sample_mas_pcc": 0.3959}}}
    (RESULTS / f"summary_{args.mode}{scope_suffix}.yaml").write_text(yaml.safe_dump(payload, sort_keys=False))
    lines = [f"# RNA encoder comparison — {args.scope} {args.mode}", "", "| encoder | seed | best epoch | headline MAS-PCC | headline MSE |", "|---|---:|---:|---:|---:|"]
    for r in rows:
    ...
    (RESULTS / f"summary_{args.mode}{scope_suffix}.md").write_text("\n".join(lines)+"\n")
