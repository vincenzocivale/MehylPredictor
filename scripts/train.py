...
def _rna(args):
    ...
    trainer = RNAMethylationTrainer(
        ...
        genomic_fm_cache=args.genomic_fm_cache,
...
def main():
    p=argparse.ArgumentParser(description=__doc__); p.add_argument("--model",choices=["rna_methylation","cpg_statistics"],required=True); p.add_argument("--scope",choices=["chr1","chr123","genomewide"],required=True); p.add_argument("--recipe",required=True); p.add_argument("--output-root",required=True); p.add_argument("--run-id",default=None); p.add_argument("--resume",action="store_true"); p.add_argument("--lr",type=float,default=None); p.add_argument("--epochs",type=int,default=None); p.add_argument("--scheduler",choices=["constant","cosine","cosine_warmup"],default=None); p.add_argument("--seed",type=int,default=None); p.add_argument("--registry",required=True); p.add_argument("--canonical-root"); p.add_argument("--dataset-source",default=None,help="single-source bundle name (e.g. encode_wgbs); enables non-TCGA canonical layout"); p.add_argument("--prior-cache",help="optional metric-only locus prior cache (cpg_idx.npy + prior.npy)"); p.add_argument("--rna-cache"); p.add_argument("--locus-store",help="canonical genome-wide CpG feature store"); p.add_argument("--functional-atlas"); p.add_argument("--annotation-cache"); p.add_argument("--bigwig-cache",help="optional complete BigWig PCA context cache"); p.add_argument("--genomic-fm-cache",help="E04 comparator: chr1-only genomic-FM embedding cache (cpg_idx + embedding HDF5), mutually exclusive with --locus-store/--functional-atlas/--annotation-cache/--bigwig-cache"); p.add_argument("--mode",choices=["development","final"],default="final"); p.add_argument("--prepared-root"); p.add_argument("--targets"); p.add_argument("--embeddings"); p.add_argument("--cpg-targets-dir",help="cpg_statistics targets dir for the mean-branch proxy task"); p.add_argument("--early-stop-patience",type=int,default=None,help="stop once the training signal hasn't improved for this many epochs"); p.add_argument("--development-split-seed",type=int,default=None,help="fix the inner development split independently of the training seed"); p.add_argument("--training-sources",default=None,help="comma-separated subset of array,epic,wgbs to restrict training to (paper section B.6 source ablation); array is always included; default is all three, unchanged from before this flag existed"); args=p.parse_args()
    if args.model=="rna_methylation":
        for name in ("canonical_root","rna_cache"):
