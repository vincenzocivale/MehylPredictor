...
- ``locus_dynamic_pearson_median``/``_spearman_median`` (per-CpG, axis=0) are
  ...
  very largest panels, same escape hatch already used by
  ``scripts/rna_branch/build_ensembles_and_compare.py``).
...
over the entire CpG axis) -- same precedent as
``scripts/rna_branch/build_ensembles_and_compare.py`` dropping expensive
Spearman diagnostics at genome-wide scale.
"""
