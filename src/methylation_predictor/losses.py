...
def residual_loss(
    ...
    # V1 experiment: standardized-residual counterparts, operating on the
    # model's raw (pre-sigma-scale) output against the sigma-normalized
    # target.  Only active when a variance-normalized model supplied both
    # sigma and outputs["raw_delta"], and at least one weight is nonzero.
    if (
        sigma is not None
