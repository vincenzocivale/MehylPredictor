"""Depth-vs-residual ablation, disentangling J0 and J1 along two axes.

J0 (``SingleRetrievalPredictor``) and J1 (``IterativeRetrievalPredictor``)
differ on THREE confounded dimensions at once: retrieval depth (1 vs 4
blocks), whether each block's cross-attention output is added residually to
the running locus state (never vs always), and parameter count. An early,
accidentally-built version of J1 (2 blocks, only the second residual) tracked
J0's training loss almost exactly, which is suggestive but not conclusive on
its own -- this module isolates the two axes cleanly:

  ``ablation_depth1_residual``    -- n_blocks=1, attn_residual=True
      Same depth as J0, but with the residual J1 has. Isolates whether the
      residual ALONE (no extra depth) already produces J1's gain.

  ``ablation_depth4_noresidual``  -- n_blocks=4, attn_residual=False
      Same depth as J1, but every block's attention output REPLACES the
      running state instead of being added to it (matching the accidental
      first J1 attempt's mechanism, applied uniformly to every block instead
      of only the first). Isolates whether depth ALONE (no residual)
      produces any gain over J0.
